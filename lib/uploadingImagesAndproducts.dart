...
//================================== showing the selected product to be updated=======================================
  ...
  Future<String> deleteProduct(int id,) async {
    ...
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? spToken = await prefs.getString('spToken');
    print('sptoken ========================$spToken');print('Got token');
    ...
    if (response.statusCode == 200) {
      try {json = jsonDecode(response.body);
        ...
        catch (e) {msgDeleteProduct = e.toString();}
    } else {msgDeleteProduct= 'Algo salió mal';}
    return msgDeleteProduct;
  }



  // Route::post('/deletedeliverman/{id}', [AdminResourceController::class, 'deletedeilveryman']);
  //  Route::post('/deleteuser/{id}', [AdminResourceController::class, 'deleteUser']);


  // public function deletedeilveryman( $id){
  //   $repartidorTobeDeleted =Repartidor::find($id);
  //   $repartidorTobeDeleted ->delete();
  //   $response = [ 'message' => 'El repartidor ha sido eliminado exitosamente ',  ];
  ...
  //   $response = [ 'message' => 'El cliente ha sido eliminado exitosamente ',  ];
  //   return Response($response, 201);
  // }


  String msgDeleteUser = '';
  Future<String> deleteUser(int id,) async {
    var json;
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? spToken = await prefs.getString('spToken');
    print('sptoken ========================$spToken');print('Got token');
    final url = Uri.parse('https://hbtknet.com/admin/deleteuser/$id');
    Map<String, String> headers = {
      'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': 'Bearer $spToken',};
    final response = await post(url, headers: headers);
    if (response.statusCode == 201) {
      try {json = jsonDecode(response.body);
      msgDeleteUser = json['message'];}
      catch (e) {msgDeleteUser = e.toString();}
    } else {msgDeleteUser= 'Algo salió mal';}
    return msgDeleteUser;
  }

  String msgDeleteDeliveryman = '';
  Future<String> deleteDeliveryman(int id,) async {
    var json;
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? spToken = await prefs.getString('spToken');
    print('sptoken ========================$spToken');print('Got token');
    final url = Uri.parse('https://hbtknet.com/admin/deletedeliverman/$id');
    Map<String, String> headers = {
      'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': 'Bearer $spToken',};
    final response = await post(url, headers: headers);
    if (response.statusCode == 201) {
      try {json = jsonDecode(response.body);
      msgDeleteDeliveryman= json['message'];}
      catch (e) {msgDeleteDeliveryman= e.toString();}
    } else {msgDeleteDeliveryman= 'Algo salió mal';}
    return msgDeleteDeliveryman;
  }
